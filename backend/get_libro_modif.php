...
$sql = "SELECT id_libro, titulo, autor, portada, id_editorial, anio_edicion, idioma, ISSN, ISBN, descriptor_primario, descriptor_secundario, numero_edicion, ubicacion, cantidad_ejemplares FROM libro";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    /* $cantidad_ejemplares = !empty($row['cantidad_ejemplares']) ? $row['cantidad_ejemplares'] : 'Cantidad ejemplares no disponible'; */
    ...
    echo "<a href='modif_libro.php?id=$id_libro&titulo=$titulo&autor=$autor&id_editorial=$id_editorial&anio_edicion=$anio_edicion&idioma=$idioma&ISSN=$ISSN&ISBN=$ISBN&descriptor_primario=$descriptor_primario&descriptor_secundario=$descriptor_secundario&ubicacion=$ubicacion&numero_edicion=$numero_edicion'>";
    echo "<button id='inspBttn' onclick='to_modif_result()'>Modificar</button>";
    echo "</a>";
    echo "</div>";
